<?php
//define ( 'CONFIG_DIR', $_SERVER ['DOCUMENT_ROOT'].PATH_PROJ_ETC);
require_once __DIR__."/../../../include/conf/conf_path.php";
define('CONFIG_DIR', PATH_PROJ_ETC);
 ...
 */
function useFastDfsClientConfig($cfg) {
	FastDFS::$clientconfig = $cfg;
}
...
if (! class_exists ( 'FastDFS', false )) {
	...
	class FastDFS {
		 ...
		 * @var unknown
		 */
		public static $clientconfig = null;
		...
		public function __construct() {
			...
			// if (FastDFS::$clientconfig == null)
			// $ini = parse_ini_file ( php_ini_loaded_file () );
			// else {
			// // echo ("使用自定义配置文件:" . FastDFS::$clientconfig);
			// $ini = parse_ini_file ( FastDFS::$clientconfig );
			// }
			//自动加载配置文件
			$ini = parse_ini_file ( CONFIG_DIR . "fastdfs.ini" );
			
			$ini ["fastdfs_client.tracker_group0"] = CONFIG_DIR . "track.conf";
			// ini_set("fastdfs_client.tracker_group0", CONFIG_DIR."track.conf");
		 ...
		 */
		public function storage_delete_file($groupName, $remoteFile, FastDFSTrackerClient $tracker, FastDFSStorageClient $storage) {
		 ...
		 *        	文件的扩展名，当名优提供扩展名时，会自动取文件的扩展名
		 * @param array $metas
		 *        	文件的附加信息
		 * @param string $groupName
		 *        	所在的组名，可以为空，为空时，由tracker决定
		 * @param FastDFSTrackerClient $tracker        	
		 * @param FastDFSStorageClient $storage        	
		 */
...
function createDownloadLink($fileurl) {
	...
	$fileurlpars = explode ( "#", $fileurl );
// 	$urlbase = null;
	$filedownloadLink = "http://172.23.178.94:81/".$fileurlpars[0]."/".$fileurlpars[1];
 ...
 * @return string
 */
function uploadFile($fileurl, $sizes = array()) {
	$dfs = new FastDFS ();
	...
	if ($dfs->active_test ( $tracker )) {
		...
		if (! empty ( $sizes )) {
			...
			$filename = $dfs->storage_upload_by_filename ( $fileurl, $ext, $defaultmeta );
			if (isset ( $filename ['group_name'] ) && isset ( $filename ['filename'] )) {
				$location = $filename ['group_name'] . "#" . $filename ['filename'];
			}
		}
 ...
 * 检测文件是否存在
 * 
 * @param unknown $fileuri        	
 * @return number
 */
function isFileExists($fileuri) {
	$fileinfo = explode ( "#", $fileuri );
	$dfs = new FastDFS ();
	$groupname = $fileinfo [0];
	$fileid = $fileinfo [1];
	$trackserver = $dfs->tracker_get_connection ();
	$storaged = $dfs->tracker_query_storage_store ( "group1", $trackserver );
	// var_dump($storaged);
	$filelive = $dfs->storage_file_exist ( $groupname, $fileid, $trackserver, $storaged );
	return $filelive;
}
...
function removeFile($fileuri){
	...
	$trackserver = $dfs->tracker_get_connection ();
	$storaged = $dfs->tracker_query_storage_store ( "group1", $trackserver );
// 	var_dump($trackserver);
	$dfs->storage_delete_file($groupname, $fileid, $trackserver, $storaged);
}
 ...
 */
function updateFile($fileid,$localfile){
 	removeFile($fileid);
 	$addfileid = uploadFile($localfile);
 	return $addfileid;
}
